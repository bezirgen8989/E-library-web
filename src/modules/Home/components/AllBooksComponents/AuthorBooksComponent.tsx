...
const AuthorsBooksComponent: React.FC<HomeProps> = ({books, getBook, title}) => {
    const history = useHistory();
    const isLoading = !books || books.length === 0; // Состояние загрузки

    return (
        <div className={styles.home_page}>
            <div onClick={() => history.goBack()} className={commonStyles.backBtnRelativePage}>
            ...
            <div className={styles.page_title}>
                {isLoading ? (
                    <Skeleton active style={{height: 70, width: 100}} title={false} paragraph={{rows: 0}}/>
                ) : (
                    <h1>{title}</h1>
                )}
            </div>
