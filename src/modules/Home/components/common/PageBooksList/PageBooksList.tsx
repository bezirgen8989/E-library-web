...
const PageBooksList: FC<PageBooksListProps> = ({ title, titleImage, books, seeAllLink, getBook }) => {
    const [displayCount, setDisplayCount] = useState<number>(books ? books.length : 0);
    ...
    return (
        <div className={styles.rowNewBooks}>
            ...
            <div className={styles.newBooksList}>
                {books && books.slice(0, displayCount).map((book) => (
                    <div key={book.id} className={styles.newBook} onClick={() => getBook(book.id)}>
                        <div className={styles.imgWrap}>
                            {book.bookCover?.link ? (
                                <img src={book.bookCover.link} alt={book.title} className={styles.bookCoverImage} />
                            ) : (
