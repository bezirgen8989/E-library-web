...
const TopBooksSlider: FC<TopBooksSlider> = ({ books, getBook }) => {
    const isLoading = !books; // Определяем состояние загрузки
            ...
            <div className={styles.topBooksList}>
                {isLoading
                    ? Array.from({ length: 3 }).map((_, index) => (
                        <div key={index} className={styles.topBook} style={{ height: "174px" }}>
                    ...
                    : books.map((book: Book) => (
                        <div key={book.id} className={styles.topBook}>
                            <div>
                                ...
                                <div className={styles.topBookAuthor}>{book.author[0]?.name}</div>
                                <div className={styles.backgroundImg}>
                                    {book.bookCover?.link ? (
                                        <img
