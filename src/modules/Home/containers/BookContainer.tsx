...
const BookContainer: React.FC = () => {
    ...
    const addToBookShelf = useCallback((params) => {
        dispatch(addToShelf(params));
    }, [dispatch]);

    const deleteFromBookShelf = useCallback((params) => {
    ...
    const habitsCategories = currentBook?.result?.categories.map((genre: {
        id: string,
        name: string,
        colour: string
    }) => genre.id).join(',');
    ...
    const getAuthorBooks = useCallback((id) => {
