...
const EnterCodeForm: React.FC <RecoverProps>= ({ onSubmit, currentEmail }) => {
  ...
  const code = watch(["digit1", "digit2", "digit3", "digit4", "digit5", "digit6"]);

  const onSubmitForm = () => {
    const codeValue = code.join(""); // Преобразование массива в строку
    onSubmit({ codeOrHash: codeValue }); // Передаем строку кода как `codeOrHash`
  };

  const handleInput = (e: React.FormEvent<HTMLInputElement>, index: number) => {
    const nextField = document.querySelector<HTMLInputElement>(`input[name=digit${index + 2}]`);
    ...
    <div style={{ display: "flex", width: '100%', height: '100vh', position: 'relative' }}>
      <div className={commonStyles.centeredWrapper}>
        ...
        <div className={commonStyles.centered}>
          ...
          <div className={styles.code_subtitle}>We’ve sent a code <br/>to {currentEmail}.</div>
