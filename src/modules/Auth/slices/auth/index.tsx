...
import {
    ...
    setUserProfile,
    uploadUserAvatar,
    authRegister, authGoogleLogin, resetUserPassword,
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(emailConfirm.fulfilled, (state, action) => {
                const { content, error } = action.payload;
                state.confirmEmailRequest = { isLoading: false, result: content, error };
            })
    ...
    async (formParams: LoginUserParams) => {
        ...
        if (!success) {
            if (status === 401) {
                ...
                const errorMessage = error?.detail || "An error occurred, please try again later.";
                notification.error({
                    message: "Login Error",
...
export const getLanguages = createAsyncThunk('api/v1/auth/languageOptions', async () => {
...
export const uploadUserPhotoId = createAsyncThunk(
    '/api/v1/media/upload',
    async (userParams: { files: File; prefix: string; postfix: string; tag: string }) => {
